...
class RecBayesianPersonalizedRankingLoss(RecommenderSystemLoss):
    ...
    def compute_loss(self, logits: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
        ...
        n_pos = int(torch.where(single_label == 0)[0][0])
        pos_logits = logits[:, :n_pos]  # [batch_size,n_pos]
        neg_logits = logits[:, n_pos:]  # [batch_size,n_neg]
        # pos_logits = logits[:, 0].unsqueeze(1)  # [batch_size,1]
        # neg_logits = logits[:, 1:]  # [batch_size,n_neg]

        diff_logits = pos_logits.unsqueeze(-1) - neg_logits.unsqueeze(1)  # [batch_size,n_pos,n_neg]
        diff_logits = diff_logits.reshape(logits.shape[0], -1)  # [batch_size,n_pos*n_neg]

        labels = torch.ones_like(diff_logits).to(diff_logits.device)
...
class LossCombiner(RecommenderSystemLoss):
    """
    It combines multiple losses. The losses are weighted by the weights passed in the constructor.
    """
    ...
    def __init__(self, losses: list, weights: list):
        ...
        self.name = 'LossCombiner'

        logging.info(f'Built {self.name} module')

    @staticmethod
    def build_from_conf(conf: dict, dataset):
        raise NotImplementedError('This class cannot be built from a config file')

    def compute_loss(self, logits: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
        """
        It computes the loss as the weighted sum of the losses passed in the constructor.
        :param logits: Logits values from the network.
        :param labels: 1-0 Labels.
        :return: The weighted sum of the losses passed in the constructor.
        """
        total_loss = torch.zeros(1, device=logits.device)
        for loss, weight in zip(self.losses, self.weights):
            total_loss += weight * loss.compute_loss(logits, labels)

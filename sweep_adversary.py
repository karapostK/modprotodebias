...
from fair.utils import generate_log_str, prepare_data, prepare_evaluators, prepare_rec_model, get_delta_conf
...
def train_val_agent():
    ...
    rec_conf = fetch_best_in_sweep(adv_config['best_run_sweep_id'], good_faith=True, preamble_path="~",
    ...
    # Data
    data_loaders, user_to_user_group, n_groups, ce_weights = prepare_data(rec_conf, adv_config)
    # Recommender Model
    rec_model = prepare_rec_model(rec_conf, data_loaders['train'].dataset)
    # Evaluators
    rec_evaluator, fair_evaluator = prepare_evaluators(adv_config, rec_conf['dataset'], n_groups,
                                                       user_to_user_group)
    ...
    repr_perturb = AddRepresentationPerturb(repr_dim=64, n_masks=n_masks, user_idx_to_mask_idx=user_idx_to_mask_idx,
                                            init_std=.1, clamp_boundaries=(0, 2))
    ...
    for curr_epoch in trange(adv_config['n_epochs']):
        ...
        if rec_results['ndcg@10'] > best_recacc_value:
            ...
            # Save
            torch.save(saving_dict, os.path.join(save_path, 'best_recacc.pth'))
        ...
        if fair_results['balanced_acc'] < worst_bacc_value:
            ...
            # Save
            torch.save(saving_dict, os.path.join(save_path, 'worst_bacc.pth'))

        # Save last
        torch.save(saving_dict, os.path.join(save_path, 'last.pth'))

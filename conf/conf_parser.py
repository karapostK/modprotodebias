...
DEF_RAY_VERBOSE = 1
DEF_TRAIN_ITERATE_OVER = 'interactions'
...
def parse_conf(conf: dict, alg: AlgorithmsEnum, dataset: DatasetsEnum) -> dict:
    ...
    if issubclass(alg.value, SGDBasedRecommenderAlgorithm):
        ...
        if 'train_iterate_over' not in conf:
            conf['train_iterate_over'] = DEF_TRAIN_ITERATE_OVER
            added_parameters_list.append(f"train_iterate_over={conf['train_iterate_over']}")
        else:
            assert conf['train_iterate_over'] in ['interactions',
                                                  'users'], f"Iterate over {conf['train_iterate_over']} not implemented"

        if conf['train_iterate_over'] == 'users':
            assert 'pos_train' in conf, "Number of positive instances ('pos_train') to samples is missing from the configuration!"

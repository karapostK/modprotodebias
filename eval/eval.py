...
class FullEvaluator:
    ...
    def get_aggr_by_group(self):
        return self.aggr_by_group
...
class FullEvaluatorDecorator(FullEvaluator):
    def __init__(self, full_evaluator: FullEvaluator):
        """
        @param full_evaluator: FullEvaluator object to decorate
        """
        self.full_evaluator = full_evaluator

        self.name = 'FullEvaluatorDecorator'
    ...
    def get_aggr_by_group(self):
        return self.full_evaluator.get_aggr_by_group()
...
class FullEvaluatorCalibrationDecorator(FullEvaluatorDecorator):
    ...
    def __init__(self, full_evaluator: FullEvaluator, item_tag_mtx: torch.Tensor, user_tag_mtx: torch.Tensor,
                 metric_name_prefix: str = 'tag', beta_smoothening: float = .01):
...
def evaluate_recommender_algorithm(alg: RecommenderAlgorithm, eval_loader: DataLoader, evaluator: FullEvaluator,
    ...
    if evaluator.get_aggr_by_group():
